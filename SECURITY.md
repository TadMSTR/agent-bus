...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/agent-bus/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[agent-bus] Security Report`.
...
- NATS credential leakage between agents or across subject namespaces
- Message injection enabling unauthorized agent-to-agent task dispatch
- Replay attacks on signed event chains
- Subject namespace escapes that allow unauthorized topic access
- Dependency vulnerabilities with a plausible exploitation path in agent-bus's usage
